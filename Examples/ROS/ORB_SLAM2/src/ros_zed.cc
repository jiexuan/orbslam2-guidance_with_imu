...
public:
    ...
    void GrabIMU(const geometry_msgs::TransformStamped::ConstPtr& msg);
...
//    	ss >> boolalpha >> igb.do_rectify;
        ...
        message_filters::Subscriber<sensor_msgs::Image> left_sub(nh, "/guidance/left/image_raw", 1);
        message_filters::Subscriber<sensor_msgs::Image> right_sub(nh, "/guidance/right/image_raw", 1);
...
//        cout<<"-----------------------------------------"<<endl;
        ...
        if(UseIMU){
//        cout<<"*****************************"<<endl;
        sub_imu = nh.subscribe("/guidance/imu", 1, &ImageGrabber::GrabIMU, &igb);
...
//        cin >> IsSaveMap;
        ...
        if(IsSaveMap == 'Y' || IsSaveMap == 'y')  
            SLAM.SaveMap("MapPointandKeyFrame1.bin");
...
void ImageGrabber::GrabIMU(const geometry_msgs::TransformStamped::ConstPtr& msg)
{
    unique_lock<mutex> lock(mMutexYawAngle);
...
//    msg->header.frame_id = "guidance";
//    msg->header.stamp    = ros::Time::now();
//    msg->transform.translation.x = imu_data->acc_x;
//    msg->transform.translation.y = imu_data->acc_y;
//    msg->transform.translation.z = imu_data->acc_z;
//    msg->transform.rotation.w = imu_data->q[0];
//    msg->transform.rotation.x = imu_data->q[1];
//    msg->transform.rotation.y = imu_data->q[2];
//    msg->transform.rotation.z = imu_data->q[3];
...
//
    float dx = msg->transform.translation.x; // X acceleration
    float dy = msg->transform.translation.y; // Y acceleration
    float dz = msg->transform.translation.z; // Z acceleration

//    float rw = msg->transform.rotation.w;
...
//    tf::matrix3x3 angular_velocity = toEulerAngle(geometry_msgs::Quaternion imu_data->q)
    float rx = msg->transform.rotation.x; // roll_rate
    float ry = msg->transform.rotation.y; // pitch_rate
    float rz = msg->transform.rotation.z; // yaw_rate
